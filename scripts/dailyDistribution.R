...
staRoutes <- list.files(path = STA_FOLDER, pattern = ".csv$", full.names = T)


#optimization_result_prefix = "REM_"
working_folder = helper.getResultPath(paste0(OPTIMIZATION_RESULT_FOLDER))
...
for(i in 1:length(staFiles)){
    ...
    helper.safeCreateFolder(paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/"))
    write.csv2(result, file = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/", staNames[i], ".csv"), row.names = F)
}


staFiles <- list.files(path = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/"), full.names = T, pattern = ".csv$")
staNames <- gsub(".csv", "", list.files(path = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/"), full.names = F, pattern = ".csv$"))
...
for(i in 1:length(staFiles)){
    ...
    helper.safeCreateFolder(paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/plot120/"))
    helper.safeCreateFolder(paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/plot60/"))
    helper.safeCreateFolder(paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/plot30/"))
    ...
    ggsave(filename=paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/plot120/", "STA_", staNames[i], "_120.png"),
           plot=e, width = 19, height = 9)

    e <- ggplot(resultFrame, aes(x=TIMESLOT60, fill=SYS)) + ggtitle(paste("STA ", staNames[i])) + geom_bar() +
        theme_minimal() + scale_fill_brewer(palette="Set1") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
        scale_x_discrete(drop=FALSE) + facet_grid(DIRECTION~.)
    ggsave(filename=paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/plot60/", "STA_", staNames[i], "_60.png"),
           plot=e, width = 19, height = 9)

    e <- ggplot(resultFrame, aes(x=TIMESLOT30, fill=SYS)) + ggtitle(paste("STA ", staNames[i])) + geom_bar() +
        theme_minimal() + scale_fill_brewer(palette="Set1") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
        scale_x_discrete(drop=FALSE) + facet_grid(DIRECTION~.)
    ggsave(filename=paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/plot30/", "STA_", staNames[i], "_30.png"),
           plot=e, width = 19, height = 9)

    #staBTS <- unique(sta$BTS[sta$ID==staNames[i]])
    #fBTS <- staBTS[1]
    #lBTS <- staBTS[length(staBTS)]
    helper.safeCreateFolder(paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables60/"))
    helper.safeCreateFolder(paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables120/"))
    ...
    write.csv2(table(resultFrame$TIMESLOT60[resultFrame$DIRECTION == lv[1]],
               ...
               file = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables60/", "STA_", staNames[i], "_NEWSTA_", lv[1], ".csv"),
               row.names = T)

    write.csv2(table(resultFrame$TIMESLOT60[resultFrame$DIRECTION == lv[2]],
                     resultFrame$SYS[resultFrame$DIRECTION == lv[2]]),
               file = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables60/", "STA_", staNames[i], "_NEWSTA_", lv[2], ".csv"),
               row.names = T)

    write.csv2(table(resultFrame$TIMESLOT120[resultFrame$DIRECTION == lv[1]],
                     resultFrame$SYS[resultFrame$DIRECTION == lv[1]]),
               file = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables120/", "STA_", staNames[i], "_NEWSTA_", lv[1], ".csv"),
               row.names = T)

    write.csv2(table(resultFrame$TIMESLOT120[resultFrame$DIRECTION == lv[2]],
                     resultFrame$SYS[resultFrame$DIRECTION == lv[2]]),
               file = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables120/", "STA_", staNames[i], "_NEWSTA_", lv[2], ".csv"),
               row.names = T)
}

staFiles <- list.files(path = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables120/"), full.names = T, pattern = ".csv$")
staNames <- gsub(".csv", "", list.files(path = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/tables120/"), full.names = F, pattern = ".csv$"))
...
for(i in 1:length(staFiles)){
    ...
    write.csv2(resultFrame, file = paste0(helper.getResultPath(OPTIMIZATION_RESULT_FOLDER), "/tagesgang/FILLTABLE120/", staNames[i],".csv"), row.names = F)
}
