...
dt <- read.csv2(file = helper.getResultPath(TEMP_TFZ_FRAME_FILEPATH), stringsAsFactors = F)
dt <- dt[dt$TOTALWEIGHT >= 500,]
#dt <- dt[!duplicated(dt[,c("TFZ", "TOTALWEIGHT", "NUM_TFZ")]),c("TFZ", "TOTALWEIGHT", "NUM_TFZ", "VMAX")]

d1 <- cbind(dt, rep(7, 19024), stringsAsFactors = F)
names(d1) <- c("TOTALWEIGHT", "BREAKCLASS", "VMAX", "TFZ", "NUM_TFZ", "I")
d2 <- cbind(dt, rep(10, 19024), stringsAsFactors = F)
names(d2) <- c("TOTALWEIGHT", "BREAKCLASS", "VMAX", "TFZ", "NUM_TFZ", "I")
d3 <- cbind(dt, rep(15, 19024), stringsAsFactors = F)
names(d3) <- c("TOTALWEIGHT", "BREAKCLASS", "VMAX", "TFZ", "NUM_TFZ", "I")
d4 <- cbind(dt, rep(9, 19024), stringsAsFactors = F)
names(d4) <- c("TOTALWEIGHT", "BREAKCLASS", "VMAX", "TFZ", "NUM_TFZ", "I")
d5 <- cbind(dt, rep(21, 19024), stringsAsFactors = F)
names(d5) <- c("TOTALWEIGHT", "BREAKCLASS", "VMAX", "TFZ", "NUM_TFZ", "I")
d6 <- cbind(dt, rep(13, 19024), stringsAsFactors = F)
names(d6) <- c("TOTALWEIGHT", "BREAKCLASS", "VMAX", "TFZ", "NUM_TFZ", "I")

dtSTA <- rbind(d1, d2, d4, d3, d6, d5, stringsAsFactors = F)
...
write.csv2(dtSTA, file = helper.getResultPath(TFZ_LIST_DTSTA), row.names = F)
